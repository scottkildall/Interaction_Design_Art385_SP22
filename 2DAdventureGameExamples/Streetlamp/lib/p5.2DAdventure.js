    ...
    draw() {
        if( !this.hasValidStates ) {
            ...
            if( this.states[this.currentState].checkForCollision(this.playerSprite) === true ) {
                ...
                // save the last position for checkCollision in the future
 ...
 //-- PRIVATE FUNCTIONS: don't call these --//   
    ...
    adjustSpriteForRoom() {
        ...
        if( this.playerSprite.position.x < -1 ) {
            ...
            setEnemySpriteX(creepSprite1, width + offset, playerSprite.position.y);
            setEnemySpriteX(creepSprite2, width + offset, playerSprite.position.y);
        }
        else if( this.playerSprite.position.x > width ) {
            playerSprite.position.x = 0;
            setEnemySpriteX(creepSprite1, 0 - offset, playerSprite.position.y);
            setEnemySpriteX(creepSprite2, 0 - offset, playerSprite.position.y);
        }
        else if( this.playerSprite.position.y < -1 ) {
            playerSprite.position.y = height;
            setEnemySpriteX(creepSprite1, playerSprite.position.x, height + offset);
            setEnemySpriteX(creepSprite2, playerSprite.position.x, height + offset);
        }
        else if( this.playerSprite.position.y > height ) {
            playerSprite.position.y = 0;
            setEnemySpriteX(creepSprite1, playerSprite.position.x, 0 - offset);
            setEnemySpriteX(creepSprite2, playerSprite.position.x, 0 - offset);
        }
    }
    ...
    constrainSpriteBounds() {
        ...
        else if(this.playerSprite.position.x > SCENE_W+1 ) {
            this.playerSprite.position.x = SCENE_W;
        }
        else if(this.playerSprite.position.y < -1 ) {
            this.playerSprite.position.y = 0;
        }
        else if(this.playerSprite.position.y > SCENE_H ) {
            this.playerSprite.position.y = SCENE_H;
        }
    }
...
// Maj's enemySprite additions
// Called in adjustSpriteForRoom
function setEnemySpriteX(spritename, x, y) {
    spritename.position.x = x;
    spritename.position.y = y;
}
...
class PNGRoom {
    ...
    // Go through our array and look to see if we are in bounds anywhere
